...
#' opchar_tau <- sim(tau = seq(-des$delta, 2*des$delta, length.out = 10))
...
#' @export
opchar <- function(des = des_gs(), tau = des$opchar$tau, summary = F) {
  ...
  opchar        <- list(des     = des,
                        ...
                        summary = summary,
                        tau     = tau)
  class(opchar) <- c(class(opchar), "OptGS_opchar")
  opchar
